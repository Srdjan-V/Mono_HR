...
export const updateModel = async (id: string, updatedModel: Partial<VehicleModel>): Promise<VehicleModel> => {
    const response = await axios.put(`${API_URL}/v1.0/vehiclemodel/${id}`, updatedModel);
    return response.data;
};
...
export const fetchOwners = async (): Promise<VehicleOwner[]> => {
    const response = await axios.get(`${API_URL}/v1.0/vehicleowner`);
    return response.data;
};

export const updateOwner = async (id: number, updatedOwner: Partial<VehicleOwner>): Promise<VehicleOwner> => {
    const response = await axios.put(`${API_URL}/v1.0/vehicleowner/${id}`, updatedOwner);
    return response.data;
};
